...
# # ECドメインから維持療法の開始日・終了日データを抽出


listec <- levels(as.factor(dxt_ec$USUBJID)) # 症例登録番号をリスト化する

for(i in 1:length(listec)) {

dxt <- dxt_ec[dxt_ec$USUBJID == listec [i], ]
if(i == 1){
    result <- data.frame(
      USUBJID = listec [i],
      ECSTDTC_first = dxt[which.min(dxt$ECSTDTC) , 16],
      ECSTDTC_last = dxt[which.max(dxt$ECSTDTC) , 16])
} else {
    df <- data.frame(
    USUBJID = listec [i],
    ECSTDTC_first = dxt[which.min(dxt$ECSTDTC) , 16],
...
colnames(result) <- c("USUBJID", "ECSTDTC_min", "ECSTDTC_max")
...
#     df <- data.frame(
#       USUBJID = listsv[i],
#       ECSTDTC_first = dxt_ec[which.min(dxt$ECSTDTC) , 16],
#       SVSTDTC = dxt_sv[dxt_sv$USUBJID == listsv[i], 7],
#       ECSTDTC_last = dxt_ec[which.max(dxt$ECSTDTC) , 16],
#       SVENDTC = dxt_sv[dxt_sv$USUBJID == listsv[i], 8]
#     )
#   }
#   result <- rbind(result, df)
